...
void Timer::printTimer(const std::string& timer_name) {
  ...
  if (timer_iter != instance.timers_.end()) {
    ...
    std::cout << "Desc: " << timer_name << " iter: " << count << " mean (ms): " << mean
              << " total(s): " << total / 1000.0f << " var{ms): " << var << std::endl;
  } else {
...
void Timer::updateTimer(const std::string& timer_name) {
  ...
  data.running = false;
  float ms = std::chrono::duration<float, std::milli>(std::chrono::steady_clock::now() - data.start)
                 .count();
  data.count += 1;
...
void Timer::internalStart(const std::string& timer_name) {
  ...
  if (timers_.find(timer_name) == timers_.end()) {
    ...
    if (timers_.at(timer_name).running) {
      std::cout << "CALLED START ON TIMER: " << timer_name << " BEFORE IT WAS STOPPED" << std::endl;
...
void Timer::internalStop(const std::string& timer_name) {
  // check if start was called on this timer
  if (timers_.find(timer_name) != timers_.end() and timers_.at(timer_name).running) {
    updateTimer(timer_name);
  } else {
    std::cout << "CALLED STOP ON TIMER: " << timer_name << " BEFORE IT WAS STARTED" << std::endl;
  }
}
