...
void FeatureManager::setDepth(const VectorXd& x) {
  ...
  for (auto& it_per_id : feature) {
    ...
    // ROS_INFO("feature id %d , start_frame %d, depth %f ", it_per_id->feature_id, it_per_id->
    // start_frame, it_per_id->estimated_depth);
...
double FeatureManager::compensatedParallax2(const FeaturePerId& it_per_id, int frame_count) const {
  ...
  // p_i_comp = ric[camera_id_j].transpose() * Rs[r_j].transpose() * Rs[r_i] * ric[camera_id_i] *
  // p_i;
  p_i_comp = p_i;
  double dep_i = p_i(2);
