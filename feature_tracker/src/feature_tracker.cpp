...
void FeatureTracker::readImage(const cv::Mat& _img, double _cur_time) {
  ...
  if (PUB_THIS_FRAME) {
    ...
    if (n_max_cnt > 0) {
      ...
      std::vector<cv::KeyPoint> fast_pts;
      cv::FAST(cur_img, fast_pts, 20, true);
      cv::KeyPointsFilter::retainBest(fast_pts, MAX_CNT - forw_pts.size());
      cv::KeyPoint::convert(fast_pts, n_pts);
