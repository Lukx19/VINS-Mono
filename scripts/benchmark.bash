...
do
    ...
    then
        ...
        # instakl this to strack cpu and memory usage pip install psrecord
        psrecord "${executable_cmd} | tee ${output_path}/output_${dataset}.log" --log "${output_path}/cpu_mem_${dataset}.txt" --plot "${output_path}/cpu_mem_${dataset}.png" --include-children --interval 1
        # time ${executable_cmd}

    fi
    python ${current_dir}/evaluate_ate_rpe.py $ground_truth $recording_file --plot --verbose --save_associations --results_dir $output_path --prefix $dataset >> "${output_path}/results.log"
